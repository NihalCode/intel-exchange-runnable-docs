...
export function CredentialManager() {
  ...
  async function connect(productId: ProductId) {
    ...
    if (!form?.baseUrl || !form.accessId || !form.secretKey) return;
    setBusy(productId);
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      {PRODUCTS.map((product) => {
        ...
        return (
          <section key={product.id} className="rounded-xl border border-zinc-200 p-5 dark:border-zinc-800">
            ...
            {credential ? (
              <div className="mt-4 space-y-1 text-xs text-zinc-600 dark:text-zinc-400">
                ...
                <p>Last validated: {credential.validatedAt ? new Date(credential.validatedAt).toLocaleString() : "Never"}</p>
                <p>Expires: {credential.expiresAt ? new Date(credential.expiresAt).toLocaleString() : "Provider managed"}</p>
                <p>Scopes: {credential.authorizedScopes.length ? credential.authorizedScopes.join(", ") : "Not reported"}</p>
              </div>
            ) : null}
            <div className="mt-4 space-y-3">
              <label className="block text-xs">
                <span>Environment / Base URL</span>
                <input value={form.baseUrl} onChange={(event) => update(product.id, "baseUrl", event.target.value)} placeholder="https://your-tenant.cyware.com/…" className="mt-1 w-full rounded-md border border-zinc-300 bg-transparent px-3 py-2 text-sm dark:border-zinc-700" />
              </label>
              <label className="block text-xs">
                <span>Access ID</span>
                <input value={form.accessId} onChange={(event) => update(product.id, "accessId", event.target.value)} autoComplete="off" className="mt-1 w-full rounded-md border border-zinc-300 bg-transparent px-3 py-2 text-sm dark:border-zinc-700" />
              </label>
              <label className="block text-xs">
                <span>Secret Key</span>
                <input type="password" value={form.secretKey} onChange={(event) => update(product.id, "secretKey", event.target.value)} autoComplete="new-password" className="mt-1 w-full rounded-md border border-zinc-300 bg-transparent px-3 py-2 text-sm dark:border-zinc-700" />
              </label>
            </div>
            {message[product.id] ? <p className="mt-3 text-xs" role="status">{message[product.id]}</p> : null}
            <div className="mt-4 flex gap-2">
              <button type="button" disabled={busy === product.id} onClick={() => void connect(product.id)} className="rounded-md bg-sky-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-50">
                {busy === product.id ? "Testing…" : "Test & connect"}
              </button>
              {credential ? (
                <button type="button" disabled={busy === product.id} onClick={() => void disconnect(product.id)} className="rounded-md border border-zinc-300 px-3 py-2 text-xs dark:border-zinc-700">
                  Disconnect
                </button>
              ) : null}
            </div>
