  ...
  switch (code) {
    case "OKTA_NOT_CONFIGURED":
      return {
        ...
        hint:
          "Set OKTA_ORG_URL, OKTA_API_TOKEN, and OKTA_APP_ID so Add user can create people in Okta.",
      };
    case "OKTA_UNAVAILABLE":
      return {
        status: 502,
        error: "Okta API is unavailable.",
        hint: detail ?? "Verify OKTA_ORG_URL and OKTA_API_TOKEN (ssws token with users/apps scopes).",
      };
