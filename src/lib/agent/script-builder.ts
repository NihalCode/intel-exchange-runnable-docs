...
Standalone runnable workflow generated by the Cyware Intel Exchange docs agent.
Auth, retries/rate-limit backoff, error handling and step chaining are built in.

Usage:
    export CTIX_ACCESS_ID=...      # from API Settings > Integrators
    export CTIX_SECRET_KEY=...
    python workflow.py
"""
import base64, hashlib, hmac, json, os, re, sys, time
import urllib.parse, urllib.request, urllib.error

BASE_URL = os.environ.get("CTIX_BASE_URL", "BASE_URL_PLACEHOLDER").rstrip("/")
ACCESS_ID = os.environ.get("CTIX_ACCESS_ID", "")
SECRET_KEY = os.environ.get("CTIX_SECRET_KEY", "")

MAX_RETRIES = 4
TIMEOUT = 30

if not ACCESS_ID or not SECRET_KEY:
    sys.exit("Set CTIX_ACCESS_ID and CTIX_SECRET_KEY environment variables.")
...
function buildPython(plan: ScriptPlan, baseUrl: string, title: string): string {
  const body = plan.ops
    .map((op) => (op.type === "call" ? emitPythonCall(op) : emitPythonFindOrCreate(op)))
    .join("\n\n");
  const runtime = PY_RUNTIME.replace("TITLE_PLACEHOLDER", title.replace(/"/g, "'")).replace(
 ...
 *
 * Usage:
 *   export CTIX_ACCESS_ID=...
 *   export CTIX_SECRET_KEY=...
 *   node workflow.js
 */
const crypto = require("node:crypto");

const BASE_URL = (process.env.CTIX_BASE_URL || "BASE_URL_PLACEHOLDER").replace(/\\/$/, "");
const ACCESS_ID = process.env.CTIX_ACCESS_ID || "";
const SECRET_KEY = process.env.CTIX_SECRET_KEY || "";
const MAX_RETRIES = 4;
const TIMEOUT_MS = 30000;

if (!ACCESS_ID || !SECRET_KEY) {
  console.error("Set CTIX_ACCESS_ID and CTIX_SECRET_KEY environment variables.");
  process.exit(1);
}
...
function buildJavascript(plan: ScriptPlan, baseUrl: string, title: string): string {
  const body = plan.ops
    .map((op) => (op.type === "call" ? emitJsCall(op) : emitJsFindOrCreate(op)))
    .join("\n\n");
  const runtime = JS_RUNTIME.replace("TITLE_PLACEHOLDER", title.replace(/\*\//g, "* /")).replace(
    "BASE_URL_PLACEHOLDER",
    baseUrl
  );
  ...
  for (const lang of languages) {
    if (lang === "python") {
      scripts.push({
        ...
        code: buildPython(plan, baseUrl, title),
        notes: plan.notes,
      });
      ...
      scripts.push({
        ...
        code: buildJavascript(plan, baseUrl, title),
        notes: plan.notes,
      });
    }
  }
